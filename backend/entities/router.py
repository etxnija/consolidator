...
def create_entity(
    ...
    if body.parent_entity_id is not None:
        parent = (
            ...
            .filter(EntityMetadata.entity_id == body.parent_entity_id, EntityMetadata.tenant_id == tenant_id)
            .one_or_none()
        )
        if parent is None:
            raise HTTPException(
...
def get_entity(
    ...
    entity = (
        db.query(EntityMetadata)
        .filter(EntityMetadata.entity_id == entity_id, EntityMetadata.tenant_id == tenant_id)
        .one_or_none()
    )
    if entity is None:
